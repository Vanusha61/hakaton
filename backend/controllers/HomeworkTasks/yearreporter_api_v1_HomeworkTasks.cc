#include "yearreporter_api_v1_HomeworkTasks.h"
                          ...
                          const yearreporter::services::SolvedTasksCountResult &result)
{
    ...
    json["solved_homework_tasks_count"] = result.solvedTasksCount;
    return json;
}
...
void homework_tasks::quarter(const drogon::HttpRequestPtr &request,
                             std::function<void(const drogon::HttpResponsePtr &)> &&callback)
{
    ...
    const auto result =
        service.computeHomeworkSolvedTasksQuarter(drogon::app().getDbClient(),
                                                  *quarterValue,
                                                  *userId,
                                                  *courseId,
                                                  error);
...
void homework_tasks::year(const drogon::HttpRequestPtr &request,
                          std::function<void(const drogon::HttpResponsePtr &)> &&callback)
{
    using namespace yearreporter::controllers::common;
    yearreporter::services::ApiError error;
    ...
    yearreporter::services::TaskStatsService service;
    const auto result =
        service.computeHomeworkSolvedTasksYear(drogon::app().getDbClient(), *userId, *courseId, error);
    if (!result.has_value())
    {
        callback(makeErrorResponse(error.status, error.reason));
        return;
