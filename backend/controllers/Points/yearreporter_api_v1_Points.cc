#include "yearreporter_api_v1_Points.h"
...
void points::quarter(const drogon::HttpRequestPtr &request,
                     std::function<void(const drogon::HttpResponsePtr &)> &&callback)
{
    ...
    const auto metrics =
        service.computePointsQuarter(drogon::app().getDbClient(), *quarterValue, *userId, *courseId, error);
    ...
    json["earned_count"] = metrics->valueCount;
    json["total_count"] = metrics->totalCount;
    callback(makeJsonResponse(json));
}

void points::year(const drogon::HttpRequestPtr &request,
                  std::function<void(const drogon::HttpResponsePtr &)> &&callback)
{
    using namespace yearreporter::controllers::common;
    yearreporter::services::ApiError error;
    ...
    yearreporter::services::CourseMetricsService service;
    const auto metrics =
        service.computePointsYear(drogon::app().getDbClient(), *userId, *courseId, error);
    if (!metrics.has_value())
    {
        callback(makeErrorResponse(error.status, error.reason));
        return;
