#include "yearreporter_api_v1_Videos.h"
...
void videos::quarter(const drogon::HttpRequestPtr &request,
                     std::function<void(const drogon::HttpResponsePtr &)> &&callback)
{
    ...
    const auto metrics =
        service.computeVideoQuarter(drogon::app().getDbClient(), *quarterValue, *userId, *courseId, error);
    ...
    json["watched_count"] = metrics->valueCount;
    json["total_count"] = metrics->totalCount;
    callback(makeJsonResponse(json));
}

void videos::year(const drogon::HttpRequestPtr &request,
                  std::function<void(const drogon::HttpResponsePtr &)> &&callback)
{
    using namespace yearreporter::controllers::common;
    yearreporter::services::ApiError error;
    ...
    yearreporter::services::CourseMetricsService service;
    const auto metrics =
        service.computeVideoYear(drogon::app().getDbClient(), *userId, *courseId, error);
    if (!metrics.has_value())
    {
        callback(makeErrorResponse(error.status, error.reason));
        return;
